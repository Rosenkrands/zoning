...
params <- bind_rows(
  expand.grid(names(instances),c("GA"),c("TOT"), dimensions,c(rep(100, 5),rep(200,5))),
...
solve_n_save <- function(param) {
  ...
  dimension <- as.character(param$dimension); miter <- as.numeric(param$miter)
  
  sol_id <- paste0(sample(c(0:9, LETTERS[1:6]), 5, T), collapse = '')
  
  file = paste0(
    './dimension_tuning/',instance,'_',
    method,'_',obj,'_',dimension,'_',miter,'_',sol_id,'.rds'
  )
